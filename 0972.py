    ...
    for tupl in lst:
        ...
        for el in tupl:
            ...
            if isinstance(el, int):
                summa += el
        print(" руб.")
    discount = kwargs["discount"] / 100
    print(f"Сумма: {summa} руб.")
    print(f"Скидка {kwargs["discount"]}%: -{int(summa - (summa - (summa * discount)))} руб.")
    print(f"Доставка: {kwargs["delivery"]} руб.")
    print(f"Итого: {int(summa - (summa * discount) + kwargs["delivery"])} руб.")
